...
"""
Multi-process mesh generation - each key runs in its own process(es).
...
QUEUE_FILE = SCRIPT_DIR / "mesh_queue.json"
LOCK_FILE = SCRIPT_DIR / "mesh_queue.lock"
...
def main():
    # Load manifests
    manifest_files = ["mesh_manifest_robotics_expanded.json", "mesh_manifest_combined.json"]
    all_meshes = []
    seen_ids = set()
