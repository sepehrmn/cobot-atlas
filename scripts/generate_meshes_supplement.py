...
"""
Supplemental mesh generation - runs parallel workers for specific keys (e.g. quanticsim).
"""
...
OUTPUT_DIR = SCRIPT_DIR / "output_robotics"
KEYS_FILE = SCRIPT_DIR / "api_keys.txt"
# Use a separate queue file to avoid conflict with the main process
QUEUE_FILE = SCRIPT_DIR / "mesh_queue_supplement.json"
LOCK_FILE = SCRIPT_DIR / "mesh_queue_supplement.lock"

# List of keys to target specifically for this run
TARGET_KEYS = ["quanticsim"]
    ...
    with open(KEYS_FILE) as f:
        for line in f:
            ...
            if line.startswith('#'):
                ...
                # Check if this key belongs to one of our target users
                is_target = any(target in current_name.lower() for target in TARGET_KEYS)
                if is_target:
...
def main():
    # Load specific manifest for this supplemental run
    manifest_files = ["mesh_manifest_robotics_expanded.json"]
    all_meshes = []
    seen_ids = set()
    ...
    print(f"Loaded {len(all_meshes)} mesh definitions for supplement")
    ...
    # Load keys
    keys = load_keys()
    print(f"Loaded {len(keys)} target keys ({', '.join(k['name'] for k in keys)})")
    
    if not keys:
        print("No target keys found! Check TARGET_KEYS and api_keys.txt")
        return

    # Spawn workers: 5 workers per key for this supplement
    workers = []
    ...
    print(f"\nSpawning {len(workers)} worker processes...")
    print("=" * 60)
