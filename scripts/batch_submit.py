...
"""
Batch mesh generation - submits all tasks and monitors completion.
Uses fal-ai/gpt-image-1.5 for images, fal-ai/trellis for meshes.
"""
...
MESH_MODELS = {
    "trellis": {"id": "fal-ai/trellis", "param": "image_url"},
    "hunyuan3d": {"id": "fal-ai/hunyuan3d-v3/image-to-3d", "param": "input_image_url"},
}
PRIMARY_MESH = "trellis"
...
def enhance_prompt(prompt, category=""):
    extras = "single isolated object, centered, pure white background, studio lighting, 4K, sharp focus"
...
def generate_mesh(mesh_def, api_key, results_queue):
    ...
    try:
        ...
        # Generate mesh - try primary, then fallback
        ...
        for model_key in [PRIMARY_MESH, FALLBACK_MESH]:
            ...
            try:
                mesh_result = fal_client.subscribe(
                    model_cfg["id"],
                    arguments={model_cfg["param"]: image_url},
                )
                # Extract GLB URL (different response formats)
                glb_url = (
    ...
    def process_results():
        while True:
            try:
                ...
                if total % 10 == 0:
                    print(f"[{datetime.now().strftime('%H:%M:%S')}] {total}/{len(to_generate)} ({stats['ok']} ok, {stats['fail']} fail) | {rate:.1f}/min | ETA {eta:.1f}m")
